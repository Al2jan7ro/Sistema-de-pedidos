...
import { updateClient, type ActionResponse } from '@/app/dashboard/clients/actions';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

interface ClientEditFormProps {
    client: Client; // Recibe el cliente cargado del Server Component
}
...
export function ClientEditForm({ client }: ClientEditFormProps) {
    ...
    // 2. Manejar los toasts (notificaciones)
    useEffect(() => {
        if (state.message) {
            if (state.success) {
                toast.success(state.message || 'Cliente actualizado con éxito.');
                ...
                <Card className="border-border shadow-lg min-h-[40vw] px-6">
                    <CardHeader className="space-y-2 pb-4">
                        ...
                        <CardDescription className="text-sm text-muted-foreground">
                            Gavioty Solutions - Actualice los datos del cliente
                        </CardDescription>
                    </CardHeader>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Select name="status" defaultValue={client.status}>
                                    ...
                                    <SelectContent>
                                        {Object.values(ClientStatusEnum.enum).map(status => (
